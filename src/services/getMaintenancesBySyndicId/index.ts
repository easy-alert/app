import { ApiResponse } from "../../types"; // Certifique-se de ajustar o caminho para o seu tipo ApiResponse
import AsyncStorage from "@react-native-async-storage/async-storage";

// Chave para armazenamento no AsyncStorage
const CACHE_KEY = "maintenancesCache";

// Função para buscar os dados do Kanban
export const getMaintenancesBySyndicId =
  async (): Promise<ApiResponse | null> => {
    try {
      ...
      try {
        ...
        if (cachedData) {
          return JSON.parse(cachedData) as ApiResponse; // Retorna os dados armazenados
        }
      } catch (cacheError) {
